...
\item
  ...
  Bayesian methods for inferring posterior estimations \citep{Alsing-fast19}.
\item
  Building surrogate models,\footnote{See above, and \citep{Lavin-simulation21}.} using Bayesian
  approaches for simulation planning to optimise information
  gain,\footnote{See, for example, \citep{Cranmer-active17}.}
  or deploying emulations \citep{Boelts-flexible22}
...
\item
  ...
  inference problem \citep{Kersting-uncertainty21}.
\end{itemize}
...
support implementation of probabilistic numerical methods.\footnote{See,
  ...
  and \citep{Schmidt-probabilistic21}.}
...
\item
  ...
  computed by Bayesian SBI methods \citep{Hermans-averting21,Mishra-inferring22}.
...
\item
  ...
  approximations \citep{Delaunoy-towards22}.
\item
  ...
  knowledge \citep{Wehenkel-robust22}.
\item
  Further study of the impact of model misspecification could also help
  generate new robustness diagnostic checks \citep{Cannon-investigating22}.
\end{itemize}
...
the impact of climate change and efforts to mitigate it.\footnote{For
  example: \citep{European-destination22}.}
